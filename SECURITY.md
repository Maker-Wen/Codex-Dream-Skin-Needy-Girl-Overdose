...
- The macOS and Windows Restore flows can perform that full restart when their
  restart option is selected. On Linux, restore removes the managed launcher
  state but does not terminate an already running CDP-enabled Codex process;
  quit every Codex process and reopen it normally afterward. Merely hiding the
  theme is not equivalent to closing CDP.
...
- 完全退出该 Codex 进程，并通过普通官方入口重新启动（不经过 Dream Skin 启动器）后，
  风险窗口才结束。
- macOS 与 Windows 的 Restore 流程在选择完整重启时可以完成上述操作。Linux Restore
  只移除受管启动状态，不会终止已经带 CDP 参数运行的 Codex；执行后仍须退出全部 Codex
  进程并从官方普通入口重新打开。只隐藏主题不等于关闭 CDP。
...
5. When finished, use a full macOS/Windows Restore with restart, or quit every
   Codex process and reopen Codex from its normal official entry point. On
   Linux, run restore first, then perform that full quit and normal reopen.
...
5. 使用结束后，在 macOS/Windows 执行带重启的完整 Restore，或退出全部 Codex 进程再从
   官方普通入口启动；Linux 应先运行 Restore，再完整退出并正常重开。
