...
function Wait-DreamSkinWin32WindowEvidence {
  ...
  return [pscustomobject]@{ ProcessId = 4242; Handle = '123456'; Width = 1280; Height = 800 }
}
...
try {
  ...
  if ($null -eq $pause.Failure -or
    ...
    throw "Pause-clear failure did not close Codex, restore appearance, and reopen normally: failure=$($pause.Failure.Exception.Message); events=$($pause.Events); install=$($pause.InstallCalls); restore=$($pause.RestoreCalls); locks=$($pause.LockEnters)/$($pause.LockExits)."
  }
  ...
  if ($null -eq $foreground.Failure -or
    ...
    throw "Immediate foreground failure did not reacquire the lock and restore this launch: failure=$($foreground.Failure.Exception.Message); events=$($foreground.Events); install=$($foreground.InstallCalls); restore=$($foreground.RestoreCalls); locks=$($foreground.LockEnters)/$($foreground.LockExits)."
  }
  ...
  if ($null -eq $superseded.Failure -or
    ...
    throw "Foreground recovery overwrote a newer active-theme selection made while its lock was released: failure=$($superseded.Failure.Exception.Message); events=$($superseded.Events); install=$($superseded.InstallCalls); restore=$($superseded.RestoreCalls); locks=$($superseded.LockEnters)/$($superseded.LockExits)."
  }
} finally {
