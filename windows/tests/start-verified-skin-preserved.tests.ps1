...
function Invoke-DreamSkinStartupFixture {
  ...
  function Wait-DreamSkinWin32WindowEvidence {
    ...
    return [pscustomobject]@{ ProcessId = 909; Handle = '123456'; Width = 1280; Height = 800 }
  }
...
if (-not $rendered.Failed) {
  throw "A failed verify must still abort startup even when the skin is rendered: $($rendered.LastError)"
}
