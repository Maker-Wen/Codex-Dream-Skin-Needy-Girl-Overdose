...
- 将公开 v1.5.12 标签之后合入 `main` 的 Codex 26.730 兼容、运行时补丁，以及 Issue #14/#16 的最终修复纳入正式安装包。
- Windows、macOS 与 Linux 的运行时版本统一提升到 1.5.13。

### 修复

- Codex 26.730 使用通用输入区或新版页面结构时，验证器与渲染器现在共享完整的 L1 范围信息，避免已显示皮肤仍在 90 秒截止后回滚并重启 Codex。
- 首页路由只接受可见的原生信号，并在导航后的快速刷新之外执行独立二次确认；隐藏的旧 Home 节点不再把任务页误判为首页。
- 原生建议卡按 Codex 支持的 2–4 张响应式布局处理。0–1 张时使用功能性 Command Deck，原生容器延迟挂载或补齐卡片后会立即重新测量，并彻底移除旧容器占位，避免回弹、重叠和空白布局。
- 通用输入区回退现在要求可见的真实编辑器并拒绝 footer、toolbar、editor 与空壳类名，防止非输入区域被错误公开为 Safe CSS composer 部件。
- Codex 重启遗留失效 CDP 监听器时，启动器会校验旧 owner 已退出并安全选择下一个空闲回环端口；仍存活但身份不明的监听器继续失败关闭。
- 补齐任务页、侧边聊天、项目/差异页和 Shadow DOM 源码查看器的主题识别与透明背景。
- 修复切换会话或动态界面延迟挂载时，左侧侧栏、会话导航、工作区、环境与摘要面板及系统浮层可能短暂保持原生外观的问题；相关组件挂载后会统一重新分类。

### 性能

- 渲染器合并结构刷新、延长回退探测周期，并在输入、滚动和侧栏交互期间延后高成本整理，减少主线程争用和滚动卡顿。
- Acrylic 模式继续关闭侧栏浏览器模糊与高成本离屏动画，同时保留原生材质和 INTERNET ANGEL 外观。
