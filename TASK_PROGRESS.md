...
- [complete] The macOS dimension helper is now present in the App integrity
  ...
  `full`.
- [complete] Windows initializes only the exact managed `active-theme` legacy
  shape generated by v1.5.13 (`id=custom`, missing schema) after containment,
  no-reparse and image validation, then atomically writes numeric schema 1.
  Future/string schemas, unknown IDs and external packages remain fail closed.
- [verified] Basic local checks pass: PowerShell and Git Bash syntax; focused
  Node regressions 30/30; Windows legacy migration plus future-schema refusal;
  shared runtime sync and identical validator copies; all three platform
  payload checks; nine-source 1.5.14 consistency; staged-diff whitespace; and
  no conflict markers or unstaged changes. The first Bash attempt selected WSL
  and was discarded; the clean rerun used Git for Windows and passed.
