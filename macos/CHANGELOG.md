...
- 修复菜单栏独立升级的部署清单和事务夹具漏带新增 `localization-macos.sh` 依赖的问题，确保升级后引擎包含完整本地化运行时，并使 macOS repository regressions 与 DMG 构建使用同一文件集合。

## 1.5.14 — 2026-08-09

### 更新

- 同步上游 `main@6f789be` 的安全、主题合同、菜单栏和验证改进，同时保留本仓库的 INTERNET ANGEL 三端运行时、Issue #14/#16 修复与跨平台扩展。
- Windows、macOS 与 Linux 的运行时版本统一提升到 1.5.14。

### 修复

- 菜单栏新增 System / English / 中文语言选择并持久保存；状态、主题操作、更新、恢复/卸载和一键换肤流程会使用同一语言设置。
- 渲染器会按实际 composer 面板表面计算显式强调色的前景色，修复透明或极端强调色下按钮文字对比度不稳定的问题。
- 菜单栏「重新应用皮肤」优先热重载，失败后才进入完整启动流程；「打开 ChatGPT」继续只负责原生打开或聚焦官方 App，不隐式安装引擎或改变会话。
- 渲染器可见性验证新增 `checkVisibility()`、透明度、`content-visibility` 与视口相交检查，并在导航瞬态中有界重试。
- 自定义主题写入器对齐 `schemaVersion: 1`；图片导入在调用 `sips` 前先有界读取尺寸，正式 App/DMG 同时打包并验收该辅助脚本。
- 菜单项整理为「主题」「链接」「维护」子菜单，并加入后台更新检查、一次性系统通知和手动检查入口。
- 主题包 manifest 时间戳严格校验 RFC 3339；共享图片元数据实现重新同步到双平台。

### 内部

- 吸收上游 Windows 失败启动外观事务回滚与一键换肤冷启动会话基线修复，同时保留本仓库的自动接管、精确 Win32 窗口证据和 Acrylic 扩展。
...
- 将公开 v1.5.12 标签之后合入 `main` 的 Codex 26.730 兼容与跨平台运行时修复纳入正式构建。
- Windows、macOS 与 Linux 的运行时版本统一提升到 1.5.13。

### 修复

- 更新通用输入区与页面结构识别，保持新版 Codex 主区、侧栏、首页和输入区的主题覆盖与验证一致。
- 同步 INTERNET ANGEL 运行时兼容调整，避免新版渲染器节点变化导致主题遗漏。
- 修复切换会话或动态界面延迟挂载时，导航、工作区、环境与系统浮层可能短暂保持原生外观的问题；相关组件挂载后会统一重新分类。

## 1.5.12 — 2026-08-05

### 更新

- 同步上游 v1.5.11 的主题导入、渲染器兼容、设置页识别与发布提交绑定修复，同时保留并适配超天酱 / INTERNET ANGEL 的三端运行时、浮动侧栏与 Windows Acrylic 实现。
...
- 为 Codex 26.727 的 General Settings、现代消息节点以及主区、侧栏、首页和输入区补充安全的通用渲染器回退，并避免把搜索框或对话框输入误识别为输入区。
...
- 合并 PR #8：macOS 补齐超天酱专属覆盖层、Command Deck、像素装饰、状态动画、响应式布局与主题切换表现；JPEG 与 Pixel Cafe 主题均会启用对应覆盖层。

### 安全

- payload 的主题、CSS、图片、版本与修订值改用函数回调替换，逐字保留合法的 `$` 序列；构建阶段拒绝残留占位符与无法解析的脚本。
- 注入器与检查结果不再收集或记录页面标题、会话名和 URL。

### 修复

- payload 回归测试只执行共享渲染器探针，不会在无 DOM 的 Node.js 沙箱误执行超天酱覆盖层。
- DMG 构建同时验证 Gothic、超天酱 JPEG 与 Pixel Cafe 预设资源，挂载验收继续检查应用图标。
- 同步统一运行时图片元数据读取器、可移植 Release 前置检查和有界工具链超时。

## 1.5.7 — 2026-07-26

### 更新

- `Codex Dream Skin.app` 与 DMG 的应用图标改用参照 Pixel Cafe 背景重绘的超天酱像素图标；构建脚本从仓库内固定 PNG 生成完整 `.icns` 尺寸集，并继续在签名与组包前校验图标存在。
- 跨平台版本源与 Windows Setup 同步到 `1.5.7`。

## 1.5.6 — 2026-07-26

### 新增

- DMG 与 Windows 版同步内置「超天酱 · INTERNET ANGEL」JPEG 默认主题和 Pixel Cafe 无损 PNG 主题，新安装默认启用 JPEG 版。
- 合并上游 `v1.5.5` 的主题 ZIP 安全导入、DreamSkin.cc 一键换肤、Safe CSS 与近期 Codex 渲染兼容更新。

### 修复

- 首页验证同时支持 Codex 26.721+ 的 `.home-banners` 兄弟节点布局、旧版首子节点结构和超天酱任务预设面板。
- 一键换肤前重新激活 Codex，并沿用原生窗口就绪降级策略，避免菜单栏确认框关闭后因窗口仍处于隐藏状态而取消应用。
- 测试流程同时覆盖通用运行时、超天酱专用 Windows 覆盖层、CDP 会话生命周期及窗口就绪判断。
